...
function intro() {
  ...
  textSize(20);
  text(
    "USE THE MOUSE TO MOUVE THE PADDLE TO HIT THE BALL AND DESTROY ALL BLOCKS",
    width / 2,
    height / 2 + 10
  );
  text(
...
background(BACKGROUND_COLOR);
  ...
  for (let i = 0; i < bricks.length; i++) {
    bricks[i].show();
    bricks[i].checkCollision(ball, i);
  }
...
function createBricks() {
    ...
    for (let i = 0; i < rows; i++) {
      for (let j = 0; j < cols; j++) {
        ...
        //let color = BRICK_COLORS[Math.floor(Math.random() * BRICK_COLORS.length)];
